...
Проектные правила для AI-агентов и следующих сессий зафиксированы в [`AGENTS.md`](./AGENTS.md). В частности, там описаны обязательная структура `www/js/` и `www/css/`, cache busting через `?v=` и правила обновления revision-токена.
...
## Структура клиентских ассетов

- JavaScript сайта хранится в `www/js/`.
- CSS сайта хранится в `www/css/`.
- В корне `www/` не должно быть `*.js` и `*.css`.
- Все локальные подключения JS/CSS используют общий cache-busting revision `?v=<revision>`.
- После изменения любого JS/CSS-файла revision меняется во всех HTML-страницах и динамических подключениях.
- Эти правила дополнительно проверяются Playwright-тестами в `tests/infrastructure/`.
...
- `infrastructure/` — Yarn, lockfile, структура ассетов, cache busting, Playwright и CI-инварианты;
