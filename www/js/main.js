...
// Берём общий revision клиентских ассетов из URL самого main.js, чтобы не
// дублировать вручную токен, уже заданный в HTML.
const mainScript = document.currentScript;
const ASSET_REVISION = mainScript?.src
  ? new URL(mainScript.src).searchParams.get('v') || ''
  : '';
